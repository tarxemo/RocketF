...
import { AlertTriangle, AlertCircle, AlertOctagon, Info, Bell, Flame, Navigation, Cpu, Target, Satellite, CheckCircle2, Compass } from 'lucide-react';
...
const AlertSystem: React.FC<AlertSystemProps> = ({ telemetry }) => {
  ...
  const getAlertIcon = (type: string) => {
    ...
    switch (type) {
      case 'critical': return <AlertOctagon className={`${iconClass} text-red-500`} />;
      case 'warning': return <AlertTriangle className={`${iconClass} text-yellow-500`} />;
      case 'caution': return <AlertCircle className={`${iconClass} text-orange-500`} />;
      case 'info': return <Info className={`${iconClass} text-blue-500`} />;
      default: return <Bell className={`${iconClass} text-cyan-500`} />;
    }
  };
  ...
  const getCategoryIcon = (category: string) => {
    ...
    switch (category) {
      case 'engine': return <Flame className={`${iconClass} text-red-400`} />;
      case 'flight': return <Navigation className={`${iconClass} text-blue-400`} />;
      case 'navigation': return <Compass className={`${iconClass} text-cyan-400`} />;
      case 'system': return <Cpu className={`${iconClass} text-green-400`} />;
      case 'mission': return <Target className={`${iconClass} text-yellow-400`} />;
      default: return <Satellite className={`${iconClass} text-purple-400`} />;
    }
  };
          ...
          displayAlerts.reverse().map(alert => (
            <div 
              ...
              className={`border rounded-lg p-3 transition-all duration-300 ${
                ...
                alert.type === 'caution' ? 'bg-orange-500/10 text-orange-400' : 
